...
class InterpretationTemplate:
    ...
    def generate(self, value: float) -> str:
        ...
        for threshold_name, threshold_value in sorted_thresholds:
            if (not self.inverted and value >= threshold_value) or (self.inverted and value <= threshold_value):
...
class MetricScore(BaseModel):
    ...
    def safe_compute_metric(
        ...
        try:
            ...
            if auto_invert_for_display and (0.0 <= value <= 1.0):
    ...
    @staticmethod
    def create_with_interpretation(*, value: float, sample_size: int = 1,
                                   template_key: str | None = None,
                                   custom_interpretation: str | None = None,
                                   **kw) -> "MetricScore":
        return MetricScore.safe_compute_metric(lambda: value,
                                               sample_size=sample_size,
...
class EnhancedConstraintTypeRegistry:
    ...
    # Advanced fuzzy matching patterns (fixed regex)
    _fuzzy_patterns = {
    ...
    def categorize_with_confidence(cls, constraint_text: str) -> Tuple[ConstraintCategory, float]:
        ...
        for keywords, category, weight in cls._keyword_patterns:
            score = sum(weight for keyword in keywords if keyword in constraint_lower)
            category_max[category] = weight * len(keywords)
            if score > 0:
                category_scores[category] = score
        ...
        if category_scores:
            ...
            # Confidence based on score relative to maximum possible for that category
            confidence = min(0.95, max_score / category_max[best_category])
...
# Helper function to recursively iterate over MetricScores
def _iter_metrics(obj: Any) -> Iterable[MetricScore]:
    if isinstance(obj, MetricScore):
        yield obj
    elif isinstance(obj, dict):
        for v in obj.values():
            yield from _iter_metrics(v)
